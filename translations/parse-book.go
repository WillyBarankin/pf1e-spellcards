...
var (
	headerRe   = regexp.MustCompile(`<h3 id=".*">(.*)</h3>`)
	propertyRe = regexp.MustCompile(`<p><strong>(.*)</strong>(.*)</p>`)
)
...
func main() {
	...
	for scanner.Scan() {
		...
		if p := headerRe.FindStringSubmatch(line); p != nil {
			printSpell(s, writer)
			s = Spell{}
			...
			state = PARSE_NAME
		} else if p := propertyRe.FindStringSubmatch(line); p != nil {
