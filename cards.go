...
const (
	allowedSourcePFRPG    = "PFRPG Core"
	allowedSourceUltimate = "Ultimate Magic"
)
...
var coneRangeToImg = map[string]string{
	"15 ft.": "cone-15",
	"30 ft.": "cone-30",
	"60 ft.": "cone-60",
}
...
func GetAreaImg(rng, area string) string {
	if strings.HasPrefix(area, "cone-shaped") {
		return coneRangeToImg[rng]
	}
	for _, pair := range []struct {
		substr, img string
	}{
		{"5-ft.-radius", "radius-5"},
		{"10-ft.-radius", "radius-10"},
		{"20-ft.-radius", "radius-20"},
		{"30-ft.-radius", "radius-30"},
	} {
		if strings.Contains(area, pair.substr) {
			return pair.img
		}
	}
	return ""
...
var savingThrowRepl = []struct{ from, to string }{
	{"Fortitude", "Стойкость"}, {"Fort.", "Стойкость"}, {"Fort", "Стойкость"},
	{"Will", "Воля"}, {"Reflex", "Рефлекс"},
	{"none", "нет"}, {"negates", "отменяет"}, {"partial", "частично"},
	{"half", "наполовину"}, {"disbelief", "недоверие"}, {"harmless", "безвредно"},
	{"object", "объект"}, {"blinding only", "только ослепление"},
	{"if interacted with", "при взаимодействии"},
	{"; see text", ""}, {"see text", "см. текст"}, {"or", "или"},
}

func GetSavingThrow(str string) string {
	for _, r := range savingThrowRepl {
		str = strings.ReplaceAll(str, r.from, r.to)
	}
	if str == "нет" {
		return ""
	}
...
func GenerateCards(writer io.Writer, class string, level IntSet) error {
	...
	for {
		...
		curLevel, err := GetLevel(getVal(line, class))
		if err != nil {
			return errors.New("can't parse level value: " + err.Error())
		}
		src := getVal(line, "source")
		if !level.Contains(curLevel) || (src != allowedSourcePFRPG && src != allowedSourceUltimate) {
			continue
		}

		shortDescr := strings.TrimSpace(getVal(line, "short_description_ru", "short_description"))
		descr := getVal(line, "description_formated_ru", "description_formated")
		castTime := T(getVal(line, "casting_time"))
		if castTime == "основное действие" {
			castTime = ""
		}

		spell := Spell{
			Name:           getVal(line, "name_ru", "name"),
			School:         getVal(line, "school"),
			ShortDescr:     shortDescr,
			CastTime:       castTime,
			Duration:       T(getVal(line, "duration")),
			Save:           GetSavingThrow(getVal(line, "saving_throw")),
			Range:          T(getVal(line, "range")),
			Area:           T(getVal(line, "area")),
			AreaImg:        GetAreaImg(getVal(line, "range"), getVal(line, "area")),
			Description:    template.HTML(descr),
			Level:          curLevel,
			Components:     GetComponents(getVal(line, "components")),
			ComponentsText: GetComponentsText(getVal(line, "components_ru", "components"), getVal(line, "costly_components")),
		}
		if descStr := getVal(line, "descriptor"); descStr != "" {
			spell.Descriptors = strings.Split(descStr, ", ")
		}
		data.Spells = append(data.Spells, spell)
	}
	sort.Slice(data.Spells, func(i, j int) bool { return data.Spells[i].Level < data.Spells[j].Level })
